...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/28 23:31:02 by sapark            #+#    #+#             */
/*   Updated: 2019/09/29 19:11:14 by sapark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	sha224_algo(int i, t_sha *sha)
{
	uint	s0;
	uint	s1;
	uint	ch;
...
static void	sha224_setting(char *s, t_sha *sha)
{
	int			i;
...
void		sha224(char *s)
{
	...
	sha224_setting(s, &sha);
	set = -1;
	while (++set < (int)(sha.set))
		sha224_process(set, &sha);
	set = -1;
	while (++set < 7)
		ft_printf("%.8x", sha.h_[set]);
	free(sha.n);
}
